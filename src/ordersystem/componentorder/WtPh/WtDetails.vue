<template>
  <div class="bank-details">
    ...
    <div class="bank-details-msg">
      ...
      <!--</div>-->
      <div class="msg">
        <div><span class="left">编号</span><span class="right">{{bankDetails.id}}</span></div>
        <div><span class="left">建立人</span><span class="right">{{bankDetails.erpCreator}}</span></div>
        <div><span class="left">建立时间</span><span class="right">{{bankDetails.createTs}}</span></div>
      </div>
      <div class="msg">
        <div><span class="left">受托人</span><span class="right">广东玉兰集团股份有限公司</span></div>
        <div><span class="left">名称</span><span class="right">{{bankDetails.cname}}</span></div>
        <div><span class="left">联系人</span><span class="right">{{bankDetails.customerAgent}}</span></div>
        <div><span class="left">电话</span><span class="right">{{bankDetails.officeTel}}</span></div>
      </div>
      <div class="fkpz">
        <div class="fkpz-title">委托人来图定制要求</div>
        <div class="pic-item" v-for="items in bankDetails.abdrImage">
          <img :src="'http://14.29.223.114:10250/upload' + items.imagePath" alt="" class="bank-img" @click="checkPic(items.imagePath)">
          <div class="pic-item-gg">规格：{{items.specifications}}</div>
          <div>定制要求：{{items.memo}}</div>
        </div>
      </div>
      <!--查看图片-->
      <van-popup v-model="showPic"><img ref="bigpic" :src="bigPicUrl" alt="" class="bank-img-big"></van-popup>
      <div class="msg statement">
        <div>委托人来图版权保证声明</div>
        <p>本人确认对其提供给受托人用于产品定制的图片拥有著作权或使用权，已保证其来源合法。受托人仅
          按照本人提供的图片进行相应定制提供给本人，无义务对本人提供的图片进行实质审查。 本人因该图片的
          著作权或使用权问题与第三方发生纠纷，受托人将不承担任何的法律责任，由此造成的损失由本人全部承担。</p>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //查看大图
      checkPic (url) {
        // console.log(this.$refs.bigpic.src)
        this.bigPicUrl = 'http://14.29.223.114:10250/upload' + url
        this.showPic = true
      },
      //  删除凭证
      toAgree (state) {
          let url = this.capitalUrl + '/AirbrushDesignerAssure/updateAirbrushDesignerAssure.do'
