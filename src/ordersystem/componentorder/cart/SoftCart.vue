  ...
  <div class="wall-cart">
      ...
      <div class="single-product" v-for="(group,index) in cartlist" :key="index">
        ...
        <div class="details-content" v-for="(product,inndex) in group.commodities" :key="inndex">
          ...
          <table @click="wallDetails(index,inndex)">
            ...
            <tr>
              <th>小计：</th>
              <td class="price" v-if="product.quantity">￥{{(product.quantity * product.price).toFixed(2)}}</td>
              <td class="price" v-if="!product.quantity">￥{{(product.width * product.height * product.price).toFixed(2)}}</td>
            </tr>
          </table>
    ...
    <div class="cart-bottom">
      ...
      <div class="cart-right" v-if="!showSubmitCheck&&!showManage">
        <span>合计：</span><span v-if="showPrice" class="total-price">￥{{totalPrice}}</span><span v-else
    ...
    methods: {
      ...
      pickOne(product, index, inndex) {
        if (this.cartlist[index].commodities[inndex].checked) {
          ...
          //触发全选
          if (this.checkBoxModel.length != this.cartlist[index].commodities.length) {
          ...
          } else if (this.checkBoxModel.length == this.cartlist[index].commodities.length) {
            this.checkGroupModel = []
            this.checkGroupModel.push(this.cartlist[index])
            this.cartlist[index].checked = true
          }
        }
        console.log("checkGroupModel:", this.checkGroupModel.length)
        console.log("checkBoxModel:", this.checkBoxModel.length)
        console.log("group:", this.thisGroup)
      },
      //选择某一组购物车
          ...
          if (this.checkGroupModel.length == 1) {
            if (this.thisGroup == '') {
              ...
              Dialog.alert({
                ...
                // console.log(this.checkGroupModel)
      ...
      cartList() {
        this.loading = true
        let cartUrl = this.orderBaseUrl + "/cart/getAllCartByCID.do?" +
      ...
      totalPrice: function () {
        ...
        for (var i = 0; i < this.checkBoxModel.length; i++) {
          if (this.checkBoxModel[i].quantity) {
            this.total += this.checkBoxModel[i].price * this.checkBoxModel[i].quantity
          } else {
            this.total += this.checkBoxModel[i].price * this.checkBoxModel[i].width * this.checkBoxModel[i].height
          }
        }
...
<style scoped>
  ...
  .category-title {
    ...
    line-height: 40px;
    font-size: 15px;
    border-bottom: .5px solid rgba(153, 150, 134, 0.5);
  }
  ...
  .cart-bottom {
    width: 100%;
    height: 50px;
    border-top: .5px solid rgba(153, 150, 134, 0.5);
    border-bottom: .5px solid rgba(153, 150, 134, 0.5);
    position: fixed;
