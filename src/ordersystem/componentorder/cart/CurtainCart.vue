  ...
  <div class="wall-cart">
    ...
    <div class="all-products">
      <div class="single-product" v-for="(group,index) in cartlist">
        ...
        <div class="details-content" v-for="(product,inndex) in group.curtainCartItems">
    ...
    <div class="cart-bottom">
      <div class="cart-right" v-if="!showSubmitCheck&&!showManage">
        <span>合计：</span><span class="total-price">￥{{totalPrice}}</span>
    ...
    methods: {
      ...
      pickOne(product, index, inndex) {
        if (this.cartlist[index].curtainCartItems[inndex].checked) {
          ...
          //触发全选
          if (this.checkBoxModel.length != this.cartlist[index].curtainCartItems.length) {
          ...
          } else if (this.checkBoxModel.length == this.cartlist[index].curtainCartItems.length) {
            this.checkGroupModel = []
            this.checkGroupModel.push(this.cartlist[index])
            this.cartlist[index].checked = true
          }
      ...
      pickGroup(group, index) {
        if (group.checked) {
          ...
          if (this.checkGroupModel.length == 1) {
            ...
            this.checkBoxModel = []
            for (let i = 0; i < group.curtainCartItems.length; i++) {
              this.checkBoxModel.push(this.checkGroupModel[0].curtainCartItems[i])
      ...
      cartList() {
        this.loading = true
        let cartUrl = this.url + "/cart/getAllCartByCID.do?" +
