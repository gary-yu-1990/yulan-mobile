  ...
  <div class="all-view">
    ...
    <div class="bill-title">
      <div class="bill-title1">
        <ul>
          <li><span>客户名称：</span><span class="title-sec">{{customerInfo.customerName}}</span></li>
          <li><span>联系人/电话：</span><span
            class="title-sec">{{customerInfo.customerAgent}} {{customerInfo.officeTel}}</span></li>
          <li><span>传真：</span><span class="title-sec">{{customerInfo.fax}}</span></li>
        </ul>
      </div>
      <div class="bill-title2">
        <ul>
          <li><span>版本销售前5名：</span><span class="title-sec">{{billitem.verTop}}</span></li>
          <li><span>本客户版本销售前5名：</span><span class="title-sec">{{billitem.ctmVerTop}}</span></li>
          <li><span>备货单据号：</span><span class="title-sec">{{billitem.ctmBhBill}}</span></li>
        </ul>
      </div>
      <div class="bill-title3">
        <ul>
          <li><span>实际发货总金额(本期/本年)：</span><span
            class="title-sec">{{billitem.fhjeMonth}}/{{billitem.consignmentMoney}}</span></li>
          <li><span>实际收款金额(本期/本年)：</span><span
            class="title-sec">{{billitem.czskMonth}}/{{billitem.gatherMoneyFax}}</span></li>
          <li><span>返利发货总金额(本期/本年)：</span><span class="title-sec">{{billitem.moneyFl}}/{{billitem.moneyFlTotal}}</span>
          </li>
          <li><span>本期剩余返利：</span><span class="title-sec">{{billitem.moneyFl}}</span></li>
          <li><span>备货总金额(本期/本年)：</span><span class="title-sec">{{billitem.moneyBh}}/{{billitem.moneyBhTotal}}</span>
          </li>
          <li><span>运费总金额(本期/本年)：</span><span class="title-sec">{{billitem.freightMonth}}/{{billitem.freight}}</span>
          </li>
          <li><span>应收款(期初/期末)：</span><span class="title-sec">{{billitem.qcczysk}}/{{billitem.czysk}}</span></li>
        </ul>
      </div>
    </div>
    <div :class="{'details-list-title':true,'fix':isFixed}" id="" ref="scollctn">
      明细列表
    </div>
    <div class="alllists">
      <div class="singleItem" v-for="bill in billDetailList">
        <ul>
          <li>
  ...
  export default {
    ...
    methods: {
      getBillLists() {
        this.billDetailList = []
        this.loading = true
        let url = this.orderBaseUrl + "/customerBalance/getCustomerBalancePeriodDetailInfo.do"
        let data = {
          "cid": "C19003",
        ...
        axios.post(url, data).then(
          res => {
            ...
            for (let i = 0; i < this.billDetailList.length; i++) {
              this.billDetailList[i].dateOutStock = this.exchangeTime(this.billDetailList[i].dateOutStock)
            }
      ...
      handleScroll(){
        ...
        console.log(top,top2,document.documentElement.scrollTop)
      }
    },
    created() {
      this.getBillLists()
...
<style scoped>
  ...
  .bill-title1, .bill-title2, .bill-title3 {
    background: white;
    border-radius: 5px;
    margin: 10px 0;
    font-size: 13px;
